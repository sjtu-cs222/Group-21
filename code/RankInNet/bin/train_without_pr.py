...
from listnet import dataset_rm_pr, training
from listnet.model import ListNet

logging.basicConfig(level=logging.INFO)

def run(args):
    logging.info("Loading dataset_rm_pr")

    train = dataset_rm_pr.create_dataset(args.train)
    logging.info("loaded {} sets for training".format(len(train)))

    dev = dataset_rm_pr.create_dataset(args.dev)
    logging.info("loaded {} sets for dev".format(len(dev)))

    test = dataset_rm_pr.create_dataset(args.test)
    ...
    training.train(listnet, optimizer, train_itr, 100, dev=dev,
                   device=None)
...
if __name__ == '__main__':
    ...
    args = p.parse_args(["--train", "build/MQ2007/Fold1/train_no_pr.txt",
                         "--dev", "build/MQ2007/Fold1/vali_no_pr.txt",
                         "--test", "build/MQ2007/Fold1/test_no_pr.txt"])
